...
function gameLoop() {
	...
	if (teclaCimaPressionada != teclaBaixoPressionada) { // se o jogador
		...
		if (teclaCimaPressionada) { // se for para cima...
			...
			if (jogadorPosY < (canvas.height - barraHeight)) {// se não sair
																// da tela...
				jogadorPosY += velocidadeJogador;// muda a posição
			}
		}
	...
	// Bola
	if (bolaTempo <= 0) {// se a bola estiver em jogo, o tempo é zero (após
							// marcar ponto, a bola fica invisível por um tempo)
		if ((bolaPosX - bolaRaio) <= (jogadorPosX + barraWidth)) {// se o
			...
			if ((bolaPosY + bolaRaio > jogadorPosY)
					&& (bolaPosY - bolaRaio < jogadorPosY + barraHeight)) {// se o
																			...
																			// (eixo
																			// Y)...
				bolaParaDireita = true;// a bola muda de lado e é rebatida para
										// o oponente
			...
			if ((bolaPosY + bolaRaio > oponentePosY)
					&& (bolaPosY - bolaRaio < oponentePosY + barraHeight)) {// se o
																			...
																			// (eixo
																			// Y)...
				bolaParaDireita = false;// a bola muda de lado e é rebatida para
										// o jogador
	...
	if ((bolaPosX <= -bolaRaio) || (bolaPosX > canvas.width)) {// se a bola
																// saiu da
																// tela...
		if (bolaTempo >= 50) {// se so tempo de deixar a bola invisível
								// passou...
			...
			bolaAngulo = Math.floor(Math.random() * 21) - 10;// faz a bola ir
																...
																// direção
																// aleatória
			bolaTempo = 0;// zera o tempo de deixar a bola invisível e a
	...
	var pontosA = pontosJogador;// variável temporária para não alterar
								// pontosJogador
	var pontosB = pontosOponente;// variável temporária para não alterar
									// pontosOponente

	if (pontosA < 10) {// se o número de pontos for menor que 10, colocamos o
						// zero á esquerda
		pontosA = "0" + pontosA;
	}
	if (pontosB < 10) {// se o número de pontos for menor que 10, colocamos o
						// zero á esquerda
		pontosB = "0" + pontosB;
	}
